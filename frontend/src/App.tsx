...
interface ProbeResult {
  probe_id: string
  category: string
  verdict: 'PROVEN' | 'INCONSISTENT' | 'FAILED'
  confidence: number
  rationale: string
...
interface ReliabilityReport {
  ...
  harder_gauntlet: boolean
  recommendation: string
  category_breakdown: Record<string, { total: number; proven: number; inconsistent: number; failed: number }>
...
function GauntletWorkbench({ apiBase, onBack }: { apiBase: string; onBack: () => void }) {
  ...
  async function setTargetMode(mode: 'proven' | 'flaky' | 'failing') {
    setError('')
    try {
      await fetch('http://127.0.0.1:8020/mode', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ mode }),
      })
      if (mode === 'proven') {
        setAgentId('nimbus-proven')
        setName('Nimbus Proven Worker')
      } else {
        setAgentId('nimbus-risk')
        setName(mode === 'flaky' ? 'Nimbus Flaky Worker' : 'Nimbus Risk Worker')
      }
    } catch {
      setError('Target mode endpoint is not reachable. Start target-agent on port 8020.')
    }
  }
  ...
  async function repairTarget() {
    setError('')
    try {
      await fetch('http://127.0.0.1:8020/repair', { method: 'POST' })
      setAgentId('nimbus-repaired')
      setName('Nimbus Repaired Worker')
      setEvents(prev => ['target_repaired / controllable worker restored passing behavior', ...prev].slice(0, 18))
    } catch {
      setError('Target repair endpoint is not reachable. Start target-agent on port 8020.')
    }
  }

  async function runVet(useCache = true) {
    setBusy(true)
    setError('')
    setEvents([])
    setReport(null)
    ...
    setRepair(null)
    try {
      const res = await fetch(`${apiBase}/vet`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          target_agent: { agent_id: agentId, name, endpoint },
          use_cache: useCache,
        }),
      })
      if (!res.ok) throw new Error(await res.text())
      const accepted = await res.json() as { stream_url: string; results_url: string }
      await new Promise<void>((resolve) => {
        const es = new EventSource(`${apiBase}${accepted.stream_url}`)
        es.addEventListener('telemetry', (e) => {
          const ev = JSON.parse((e as MessageEvent).data) as { stage: string; claim_id?: string; payload?: Record<string, unknown> }
          const verdict = ev.payload?.verdict ? ` -> ${String(ev.payload.verdict)}` : ''
          setEvents(prev => [`${ev.stage}${ev.claim_id ? ` / ${ev.claim_id}` : ''}${verdict}`, ...prev].slice(0, 18))
          if (ev.stage === 'gauntlet_done') {
            es.close()
            resolve()
          }
        })
      ...
      const final = await fetch(`${apiBase}${accepted.results_url}`)
      if (!final.ok) throw new Error(await final.text())
      const nextReport = await final.json() as ReliabilityReport
      setReport(nextReport)
      const [historyRes, repairRes] = await Promise.all([
        fetch(`${apiBase}/agent/${nextReport.agent_id}/history`),
        fetch(`${apiBase}/agent/${nextReport.agent_id}/repair`),
      ])
      if (historyRes.ok) setHistory(await historyRes.json())
      if (repairRes.ok) setRepair(await repairRes.json())
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Gauntlet run failed')
    } finally {
      setBusy(false)
    }
  }
  ...
  return (
    <div style={{ minHeight: '100vh', padding: '28px', boxSizing: 'border-box' }}>
      <div style={{ maxWidth: 1180, margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 16, alignItems: 'center', marginBottom: 22 }}>
          <div>
            <div style={{ color: 'var(--muted)', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.14em', fontWeight: 600 }}>
              Gauntlet
            </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(280px, 390px) 1fr', gap: 18, alignItems: 'start' }}>
          <GlassCard padding="20px">
            <div style={{ display: 'grid', gap: 12 }}>
              <label style={{ fontSize: 11, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.12em' }}>Agent id</label>
              <input value={agentId} onChange={e => setAgentId(e.target.value)} style={{ height: 40, borderRadius: 10, border: '1px solid var(--border)', background: 'rgba(255,255,255,0.05)', color: 'var(--text)', padding: '0 12px' }} />
              <label style={{ fontSize: 11, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.12em' }}>Name</label>
              <input value={name} onChange={e => setName(e.target.value)} style={{ height: 40, borderRadius: 10, border: '1px solid var(--border)', background: 'rgba(255,255,255,0.05)', color: 'var(--text)', padding: '0 12px' }} />
              <label style={{ fontSize: 11, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.12em' }}>Endpoint</label>
              <input value={endpoint} onChange={e => setEndpoint(e.target.value)} style={{ height: 40, borderRadius: 10, border: '1px solid var(--border)', background: 'rgba(255,255,255,0.05)', color: 'var(--text)', padding: '0 12px' }} />
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 4 }}>
                <button className="pill" onClick={() => setTargetMode('proven')} style={{ height: 32, fontSize: 12 }}>Proven target</button>
                <button className="pill" onClick={() => setTargetMode('flaky')} style={{ height: 32, fontSize: 12 }}>Flaky target</button>
                <button className="pill" onClick={() => setTargetMode('failing')} style={{ height: 32, fontSize: 12 }}>Failing target</button>
                <button className="pill" onClick={repairTarget} style={{ height: 32, fontSize: 12 }}>Apply repair</button>
              </div>
              <button className="pill pill-primary" disabled={busy} onClick={() => runVet(true)} style={{ height: 48, marginTop: 8 }}>
                {busy ? 'Running gauntlet...' : 'Run gauntlet'}
              </button>
              <button className="pill" disabled={busy} onClick={() => runVet(false)} style={{ height: 36, fontSize: 12 }}>
                Run without cache
              </button>
              {error && <div style={{ color: 'var(--verdict-bad)', fontSize: 12, lineHeight: 1.5 }}>{error}</div>}
            </div>
          </GlassCard>
          ...
          <div style={{ display: 'grid', gap: 18 }}>
              ...
              {report ? (
                <div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: 16, alignItems: 'flex-start' }}>
                    <div>
                      <div style={{ color: 'var(--muted)', fontSize: 12, marginBottom: 4 }}>{report.agent_id}</div>
                      <div style={{ fontSize: 26, fontWeight: 700, letterSpacing: '-0.025em' }}>{report.name}</div>
                      <div style={{ color: 'var(--text-2)', fontSize: 13, marginTop: 8 }}>{report.recommendation}</div>
                    </div>
                    <div style={{ textAlign: 'right' }}>
                      <div style={{ fontSize: 54, fontWeight: 700, color: scoreColor(report.reliability_score), lineHeight: 1 }}>
                        {Math.round(report.reliability_score * 100)}%
                      </div>
                      <div style={{ color: 'var(--muted)', fontSize: 12, marginTop: 4 }}>reliability</div>
                    </div>
                  </div>
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 18 }}>
                    <span className="pill" style={{ height: 28, fontSize: 12 }}>{report.passed} proven</span>
                    <span className="pill" style={{ height: 28, fontSize: 12 }}>{report.inconsistent} inconsistent</span>
                    <span className="pill" style={{ height: 28, fontSize: 12 }}>{report.failed} failed</span>
                    {history && <span className="pill" style={{ height: 28, fontSize: 12 }}>
                      {history.diff.runs} run{history.diff.runs === 1 ? '' : 's'} · {history.diff.score_delta >= 0 ? '+' : ''}{Math.round(history.diff.score_delta * 100)} pts
                    </span>}
                    {report.harder_gauntlet && <span className="pill" style={{ height: 28, fontSize: 12, color: 'var(--verdict-warn)' }}>harder gauntlet: prior failure</span>}
                  </div>
                  <div style={{ marginTop: 18, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 10 }}>
                    {Object.entries(report.category_breakdown).map(([category, row]) => (
                      <div key={category} style={{ border: '1px solid var(--border)', borderRadius: 10, padding: 10, background: 'rgba(255,255,255,0.035)' }}>
                        <div style={{ color: 'var(--muted)', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.08em' }}>{category}</div>
                        <div style={{ fontSize: 13, marginTop: 6 }}>{row.proven}/{row.total} proven · {row.failed} failed</div>
                      </div>
                    ))}
                  </div>
                  <div style={{ marginTop: 16, display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                    <img src={`${apiBase}/agent/${report.agent_id}/badge.svg`} alt="Gauntlet reliability badge" style={{ height: 28 }} />
                    <code style={{ color: 'var(--muted)', fontSize: 11 }}>{`/agent/${report.agent_id}/reliability`}</code>
                  </div>
                </div>
              ) : (
                <div style={{ color: 'var(--muted)', minHeight: 120, display: 'flex', alignItems: 'center' }}>
                  Run a target agent through correctness, consistency, instruction-following, safety, and hallucination-trap probes.
                </div>
              )}
            </GlassCard>

            <div style={{ display: 'grid', gridTemplateColumns: '1fr minmax(260px, 340px)', gap: 18 }}>
              <GlassCard padding="18px">
                <div style={{ color: 'var(--muted)', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: 12 }}>Probe verdicts</div>
                <div style={{ display: 'grid', gap: 10 }}>
                  {(report?.probe_results ?? []).map(probe => (
                    <div key={probe.probe_id} style={{ border: '1px solid var(--border)', borderRadius: 10, padding: 12, background: 'rgba(255,255,255,0.035)' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12 }}>
                        <div style={{ fontWeight: 600 }}>{probe.probe_id}</div>
                        <div style={{ color: probeColor(probe.verdict), fontSize: 12, fontWeight: 700 }}>{probe.verdict}{probe.cached ? ' / cached' : ''}</div>
                      </div>
                      <div style={{ color: 'var(--muted)', fontSize: 12, marginTop: 4 }}>{probe.category} - {probe.rationale}</div>
                      <div style={{ color: 'var(--text-2)', fontSize: 12, marginTop: 8, whiteSpace: 'pre-wrap' }}>
                        {probe.transcripts[0]?.response}
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <GlassCard padding="18px">
                <div style={{ color: 'var(--muted)', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: 12 }}>Live events</div>
                <div style={{ display: 'grid', gap: 8 }}>
                  {events.length === 0 ? <div style={{ color: 'var(--muted)', fontSize: 13 }}>No run yet.</div> : events.map((ev, i) => (
                    <div key={`${ev}-${i}`} style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--text-2)' }}>{ev}</div>
                  ))}
                </div>
              </GlassCard>
            </div>
            ...
            {repair && (
              <GlassCard padding="18px">
                <div style={{ color: 'var(--muted)', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: 12 }}>Repair brief</div>
                <div style={{ color: 'var(--text)', fontSize: 14, marginBottom: 12 }}>{repair.summary}</div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                  <div>
                    <div style={{ color: 'var(--muted)', fontSize: 12, marginBottom: 8 }}>Scope of trust</div>
                    {repair.scope_of_trust.map((line, i) => (
                      <div key={i} style={{ color: 'var(--text-2)', fontSize: 12, lineHeight: 1.5, marginBottom: 6 }}>{line}</div>
                    ))}
                  </div>
                  <div>
                    <div style={{ color: 'var(--muted)', fontSize: 12, marginBottom: 8 }}>Recommended repairs</div>
                    {repair.recommended_repairs.length === 0 ? (
                      <div style={{ color: 'var(--text-2)', fontSize: 12 }}>No repairs needed for this tested scope.</div>
                    ) : repair.recommended_repairs.map((line, i) => (
                      <div key={i} style={{ color: 'var(--text-2)', fontSize: 12, lineHeight: 1.5, marginBottom: 6 }}>{line}</div>
                    ))}
                  </div>
                </div>
