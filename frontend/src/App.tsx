...
interface ReliabilityReport {
  ...
  category_breakdown: Record<
    string,
    { total: number; proven: number; inconsistent: number; failed: number }
  >;
...
const EXAMPLE_TEXT = `Intercom Fin, https://www.intercom.com/fin
...
const VERDICT_META = {
  SUPPORTED: {
    ...
    description:
      "Independent public sources (case studies, third-party reviews, published methodology) corroborate this claim. We report public substantiation, not truth — absence of evidence here is not proof a claim is false.",
  },
  SELF_REPORTED_ONLY: {
    ...
    description:
      "The claim appears only on the vendor's own surfaces (site, blog, press releases). No independent public source echoes it — a signal, not a verdict on truth.",
  },
  NO_PUBLIC_RECEIPT_FOUND: {
    ...
    description:
      "We searched the public web and could not find a receipt for this claim. That does not mean the claim is false — we report public substantiation, not truth.",
  },
};
...
function vendorStatusLabel(v: VendorResult) {
  if (v.credibility_score !== null)
    return `${Math.round(v.credibility_score * 100)}%`;
...
function vendorStatusTitle(v: VendorResult) {
  if (v.credibility_score !== null)
    return "Score out of 100 — how many claims have independent public receipts. We measure public substantiation, not truth.";
  if (v.status === "unreachable")
    return "The page could not be loaded or scraped";
  if (v.status === "no_claims_extracted")
    return "The page loaded, but no specific marketing claims were found";
...
function VendorCard({ v, animIn }: { v: VendorResult; animIn: boolean }) {
  ...
  const uniqueSources = Array.from(
    new Set(v.judgments.flatMap((j) => j.receipts)),
  );
  ...
  return (
    ...
    >
      ...
      {pct !== null && (
        <div
          title="Bar shows the substantiation score — how many claims have independent public receipts"
          style={{
            height: 2,
      ...
      {v.claims.length > 0 && (
        ...
        >
          ...
          <span style={{ fontSize: 10 }}>
            {v.claims.length} claim{v.claims.length === 1 ? "" : "s"} ·{" "}
            {v.judgments.filter((j) => j.verdict === "SUPPORTED").length}{" "}
            substantiated
          </span>
        </div>
      )}
      ...
      {uniqueSources.length > 0 && (
        <>
          ...
          >
            {sourcesOpen
              ? "Hide sources"
              : `${uniqueSources.length} web source${uniqueSources.length === 1 ? "" : "s"}`}
          </button>
          {sourcesOpen && (
            <div
              style={{
      ...
      {v.advice && (
        <button
          onClick={() => setAdviceOpen((x) => !x)}
          className="pill"
          style={{ height: 30, fontSize: 12, padding: "0 14px", marginTop: 8 }}
        >
...
function SimulateChangeButton({ apiBase }: { apiBase: string }) {
  ...
  async function simulate() {
    ...
    try {
      const claims =
        NIMBUS_CLAIM_SETS[idxRef.current % NIMBUS_CLAIM_SETS.length];
      idxRef.current += 1;
...
function GauntletWorkbench({
  ...
  async function setTargetMode(mode: "proven" | "flaky" | "failing") {
    ...
    try {
      ...
      if (mode === "proven") {
        ...
        setName(
          mode === "flaky" ? "Nimbus Flaky Worker" : "Nimbus Risk Worker",
        );
      }
    } catch {
      setError(
        "Target mode endpoint is not reachable. Start target-agent on port 8020.",
      );
    }
  }
  ...
  async function repairTarget() {
    ...
    try {
      ...
      setEvents((prev) =>
        [
          "target_repaired / controllable worker restored passing behavior",
          ...prev,
        ].slice(0, 18),
      );
    } catch {
      setError(
        "Target repair endpoint is not reachable. Start target-agent on port 8020.",
      );
    }
  }
    ...
    try {
      ...
      await new Promise<void>((resolve) => {
        ...
        es.addEventListener("telemetry", (e) => {
          ...
          setEvents((prev) =>
            [
              `${ev.stage}${ev.claim_id ? ` / ${ev.claim_id}` : ""}${verdict}`,
              ...prev,
            ].slice(0, 18),
          );
            ...
            <GlassCard padding="22px">
              {report ? (
                <div>
                    ...
                    <div style={{ textAlign: "right" }}>
                      <div
                        style={{
                          fontSize: 54,
                          fontWeight: 700,
                          color: scoreColor(report.reliability_score),
                          lineHeight: 1,
                        }}
                      >
                  ...
                  >
                    ...
                    {history && (
                      ...
                      >
                        ...
                        {history.diff.score_delta >= 0 ? "+" : ""}
                        {Math.round(history.diff.score_delta * 100)} pts
                      </span>
                    )}
                  ...
                  >
                    <img
                      src={`${apiBase}/agent/${report.agent_id}/badge.svg`}
                      alt="Gauntlet reliability badge"
                      style={{ height: 28 }}
                    />
                    <code
                ...
                >
                  Run a target agent through correctness, consistency,
                ...
                <div style={{ display: "grid", gap: 10 }}>
                  {(report?.probe_results ?? []).map((probe) => (
                    ...
                    >
                      ...
                      >
                        {probe.category} - {probe.rationale}
                      </div>
                      <div
                        style={{
                          color: "var(--text-2)",
...
export default function App() {
  ...
  const fetchResults = useCallback(async (id: string) => {
    try {
      ...
      setAnimedIn((prev) => {
        ...
        sorted.forEach((v) => n.add(v.vendor));
        return n;
      });
    } catch {
      /* ignore */
  ...
  const startAudit = useCallback(async () => {
    ...
    if (vendorUrls.length === 0) {
      setCustomError(
        "No companies found. Format: Company Name, https://website.com",
      );
      return;
    }
    setCustomError("");
    ...
    try {
      ...
      es.addEventListener("telemetry", (e) => {
        ...
        setStats((s) => ({
          ...
          totalJudgments: ev.stage.startsWith("judge")
            ? s.totalJudgments + 1
            : s.totalJudgments,
          calls: s.calls + 1,
          modelUsage: ev.model
            ? {
                ...
                [ev.model]: {
                  ...
                  noResponse:
                    (s.modelUsage[ev.model]?.noResponse ?? 0) +
                    (eventTokens === 0 ? 1 : 0),
                },
              }
            : s.modelUsage,
    ...
    (async () => {
      try {
        ...
        const data = (await res.json()) as {
          market: { claim_inflation_index: number };
        };
        if (cancelled) return;
        ...
        setMarketResult((prev) =>
          prev
            ? {
                ...prev,
                claim_inflation_index: data.market.claim_inflation_index,
              }
            : prev,
        );
      } catch {
  ...
  const sorted = [...vendors].sort(
    (a, b) => (b.credibility_score ?? -1) - (a.credibility_score ?? -1),
  );
    ...
    return (
      ...
      >
        <div style={{ maxWidth: 660, width: "100%", textAlign: "center" }}>
          <div
            className="reveal-up d-0"
            style={{ marginBottom: 18, color: "var(--accent)" }}
          >
            <GauntletLogo size={84} />
          </div>
          <h1
            className="reveal-up d-1 headline-fade"
            style={{
          ...
          >
            {activeVendors.length > 0 ? (
              <span>
                ...
                {activeVendors.length === 1 ? "vendor" : "vendors"} parsed ·
                usually 15–60s
              </span>
            ) : (
              <>
  ...
  return (
    ...
    >
      <div className="reveal-down d-0">
        <StatusStrip apiBase={API_BASE} />
      </div>

      {/* Subtle audit-in-progress shimmer just under the strip */}
      {phase === "running" && (
        <div
          className="reveal-fade d-1 progress-shimmer"
          style={{
            position: "relative",
            height: 2,
        ...
        <main style={{ flex: 1, overflowY: "auto", padding: "24px 32px 40px" }}>
          ...
          >
            <div>
              ...
              >
                Ranked by substantiation density across claims. We measure
                public substantiation, not truth.
              </div>
            </div>
            ...
            <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
              ...
              >
                {fmtMs(stats.elapsedMs)} · {fmtCost(stats.totalCost)} ·{" "}
                {stats.calls} calls
              </span>
            </div>
          </div>
