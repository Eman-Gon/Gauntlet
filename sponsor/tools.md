...
How each sponsor tool is wired into Gauntlet, where it lives in the code, and its
...
- [x] **Guild.ai** — buyer agent created/published and used to reason over Gauntlet verdict data; not embedded in backend/frontend.
- [ ] **Render** — not deployed yet.

| Sponsor | What it gives us | How Gauntlet uses it | Status |
|---|---|---|---|
...
| **Composio** | Agent action / tool execution | Post claim-change alert (Slack / GitHub issue) when the gauntlet loop fires | 🟡 key in `.env`; `notify.py` pending wire-up |
...
| **Airbyte Cloud** | Managed data movement into the warehouse | External context can sync into the same ClickHouse Cloud database as Gauntlet telemetry | 🟡 destination configured against ClickHouse; source sync optional for demo |
...
| **Guild.ai** | Buyer-agent runtime / "Most Innovative Use of Agents" prize | Published procurement agent reasons over Gauntlet verdict data and recommends a vendor | ✅ published v1.0.3; external CLI demo works, not embedded in app runtime |
...
- **Config:** `SENSO_API_KEY` (set). Org: **GAUNTLET** (free tier).
...
- **Role:** no-Docker path for syncing sponsor/external context into the same
  ClickHouse Cloud warehouse as Gauntlet telemetry.
...
- **Status:** optional for the live demo. Gauntlet does not depend on Airbyte; the
  clean story is "Airbyte brings external context beside Gauntlet's telemetry."

### Composio — act on the web
- **Role:** when the gauntlet loop detects a claim change, post the delta to Slack
  or a GitHub issue.
...
- **Role:** the procurement agent side of the marketplace. Gauntlet provides paid
  verdict data; the Guild agent reasons over substantiation and chooses a vendor.
- **Code:** [gauntlet-buyer-agent/gauntlet-buyer-agent/agent.ts](gauntlet-buyer-agent/gauntlet-buyer-agent/agent.ts).
- **Status:** published as `emanuel~gauntlet-buyer-agent` v1.0.3. CLI demo works
  by passing Gauntlet verdict JSON/summary into the Guild agent; latest run
  recommended Zendesk AI after comparing SUPPORTED ratios and tie-breakers.
- **Caveat:** not embedded inside Gauntlet backend/frontend yet; the integration is
