...
async def _gmi_chat(
    ...
    *,
    ...
    # Qwen runs a chain-of-thought scratchpad by default; the `/no_think` prefix
    # alone does NOT stop it on GMI, so it burns the whole token budget on hidden
    # reasoning and blows the cheap-tier timeout. enable_thinking=False is the
    # switch that actually disables it (mirrors the OpenAI-SDK path).
